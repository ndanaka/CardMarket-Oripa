...
const EmailVerification = ({email, setIsEmailVerifyPanel}) => {
    ...
    console.log("EmailVerification component")
    useEffect(() => {
        // sendEmail();
    },[email])

    const sendEmail = () => {
        api.post("mail/gmail-send", {email: email}).then((res) => {
  ...
  return (
    <div className="w-full flex flex-col items-center justify-center py-5 bg-white">
      ...
      />
      <h1 className="text-red-600 text-3xl font-bold mb-4">Email Verification</h1>
      ...
      <button className="bg-red-600 text-white py-2 px-4 rounded hover:bg-red-700" onClick={sendEmail}>
        Resend Verification Email
      </button>
      <p className='text-blue-500 text-base text-center py-3 cursor-pointer' onClick={() => setIsEmailVerifyPanel(false)}>Choose another email</p>
    </div>
  );
};
